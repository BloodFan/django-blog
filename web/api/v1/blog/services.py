...
class BlogService:
    ...
    def like_annotate(self, user: User) -> Value:
        ...
        return Case(
                    When(likes__user=user, likes__vote=LikeStatus.LIKE,
                         then=Value(LikeStatus.LIKE)),
                    When(likes__user=user, likes__vote=LikeStatus.DISLIKE,
                         then=Value(LikeStatus.DISLIKE)),
    ...
    def tag_queryset() -> Tag:
        return (
            Tag.objects
            .annotate(ordering_count=Count('tagarticles__article'))
            .order_by(F('ordering_count').desc()).all()[:8]
        )
    ...
    def send_message_for_admin(user: User, article: Article):
        ...
        context = {
            ...
            'url':
                f'{settings.FRONTEND_URL}admin/blog/article/'
                f'{article.id}/change/'
        }
