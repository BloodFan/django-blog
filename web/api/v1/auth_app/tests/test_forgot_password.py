...
def test_email_does_not_exist(client):
    ...
    response = client.post(path=reverse('api:v1:auth_app:reset-password'), data=payload)
...
class TestValidateUidAndToken:
    def test_falsification_uid(self, client, payload_uid_and_token):
        ...
        response = client.post(path=reverse('api:v1:auth_app:reset-password-validate'), data=payload_uid_and_token)
...
@pytest.mark.parametrize(
    ...
    (
        ({'password_1': 'test_password3210', 'password_2': 'test_password3210'}, status.HTTP_200_OK),
        (
...
def test_confirm_password(client, payload_uid_and_token, password, expected_status):
    ...
    response = client.post(path=reverse('api:v1:auth_app:reset-password-confirm'), data=payload)
...
def test_password_reset(client, user):
    ...
    response = client.post(path=reverse('api:v1:auth_app:reset-password-validate'), data=payload)

    assert response.status_code == status.HTTP_200_OK
    # валидации token и uid успешно

    new_password = {'password_1': 'new_password3210', 'password_2': 'new_password3210'}
